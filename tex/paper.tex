...
\hfill
\subfloat[Examples of blocking sets in blue.]{
\includegraphics[width=0.6\textwidth]{fig/blocking-set.pdf}%
\label{fig:blocking-set}
}
\hfill
\\
\subfloat[Examples of strongly fixed pairs.]{
...
\b v$ by inspecting only $u, v$. For example, the following result is well known \cite[Lemma
...
generalization possible when only considering $u, v$ themselves, without further inspection of
