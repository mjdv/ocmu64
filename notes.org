...
- Handle infinite coordinates in PDP knapsack.
- More efficient initial merging, especially be careful around ties.
- Replace binary searches by linear scans.
- Try swapping chunks in initial solution, using cumulative sum matrix.
- Continue glueing implementation
- average tail suffix length
- improve tail bound
