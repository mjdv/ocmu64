...
total weight of $u$, and set $\bar u = u/W_u$.
...
\begin{lemma}[Strong fixed pair]\label{lem:sfp}
    Fix $u, v$. Suppose that for each $b \in A$ we have $\sum_{a \leq b} \bar u(a) \geq \sum_{a
    \leq b} \bar v(a)$. Then, for any $w$, we have $r(w, \bar v) \leq r(w, \bar u)$.
\end{lemma}

\begin{proof}
    If $\bar u \neq \bar v$, consider the least element $a_0 \in A$ such that $\bar u(a_0) \neq
    \bar v(a_0)$. We must have $\bar u(a_0) > \bar v(a_0)$. Consider the transformation of
    $\bar u$ which ``moves'' $\bar u(a_0) - \bar v(a_0)$ of weight from $a_0$ to its successor
    $a_1$, and call this transformed function $\bar u'$. Then
    \[
        r(w, \bar u') = r(w, \bar u) - ... \leq r(w, \bar u).
    \]
    Since $\bar v$ can be obtained from $\bar u$ by a finite sequence of such transformations,
    the inequality follows.
\end{proof}

% To do, fix everything from here.
\begin{proof}
...
\begin{remark}
  When the condition in \cref{lem:sfp} does not hold, there exists a multiset
