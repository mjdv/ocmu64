...
\hfill
\subfloat[Examples of blocking sets (\cref{pfp}), preventing $u\b v$.]{
...
%% }
\caption{
  Various examples where $u$ must come before $v$ ($u\b v$), where $u,v\in B$ can freely move around and
...
\begin{remark}\label{rem:strongest}
    ...
    such that $F_{\bar u}(b) < F_{\bar v}(b)$. Furthermore suppose that there are nodes $l, m,
    r \in A$, with $m$ sitting between $b$ and its successor in $A$, and $l < (\supp(u) \cup
    \supp(v)) < r$. Then consider a node $X \in B$ which is connected to $l, m, r$. By choosing
    ...
    2F_{\bar u}(b))X(m) - (X(r) - X(l))$. By choosing $X(r), X(l)$ appropriately, we can make
    ...
    \]
    Therefore, the right choice of weights of $X$ creates a situation where $c(v, X, u) <
    \min(c(X, u, v), c(u, v, X))$. This demonstrates that if you want to show $u \b v$, and $u,
    v$ are not strongly fixed, you must consider other features of the graph than $u, v$ and
    their weights: \cref{lem:sfp} is optimal. Note: this also applies in the unweighted case,
    by taking $l, m, r$ to be sets of nodes rather than single nodes.
\end{remark}
