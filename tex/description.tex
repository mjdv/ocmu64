...
\begin{lemma}[Strongly fixed pair]\label{lem:sfp}
...
If $u \neq v$ satisfy the conditions of \ref{lem:sfp}, we call $(u, v)$ a \emph{strongly fixed
pair}. Note that for such a pair, $r(u, v) < 0$.

\begin{lemma}
    If $(u, v)$ is a strongly fixed pair, then $u \b v$.
\end{lemma}
\begin{proof}
    ...
    \leq 0$, and therefore $r(X, v) = W_v r(X, \bar v) \leq W_v r(X, u) \leq 0$. But then $c(v,
    X, u) \leq c(X, v, u) < c(X, u, v)$, a contradiction.
\end{proof}
