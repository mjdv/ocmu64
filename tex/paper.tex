...
\titlerunning{OCMu64: a solver for One-sided Crossing Minimization}
...
\EventEditors{\'{E}douard Bonnet and Pawe\l{} Rz\k{a}\.{z}ewski}
\EventNoEds{2}
\EventLongTitle{19th International Symposium on Parameterized and Exact Computation (IPEC 2024)}
\EventShortTitle{IPEC 2024}
\EventAcronym{IPEC}
\EventYear{2024}
\EventDate{September 4--6, 2024}
...
\begin{abstract}
  ...
  reductions.
  We apply these in our exact solver \texttt{OCMu64}, that directly uses branch-and-bound
...
%% }
\caption{
  Various examples of $u, v \in B$, whose neighbors 
  neighbours in $A$ (on the dashed line) have fixed positions.}%
\label{fig}
\end{figure}
...
\begin{remark}\label{rem:strongest}
    ...
    single nodes with weighted edges.
\end{remark}
...
let us see that we cannot get a `strong gluing' result analogous to \cref{lem:sfimplf}.
